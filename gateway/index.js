...
try {
const r = await axios.put(`${LIVRE_SERVICE}/livres/${req.params.id}`,
req.body)
res.status(200).json(r.data)
} catch (err) {
if (err.response?.status === 404) return res.status(404).json({ message:
'Livre non trouve' })
res.status(500).json({ message: err.message })
}
})
...
app.put('/membres/:id', async (req, res) => {
    try {
    const r = await axios.put(`${MEMBRE_SERVICE}/membres/${req.params.id}`,
    req.body)
    res.status(200).json(r.data)
    } catch (err) {
    if (err.response?.status === 404) return res.status(404).json({ message:
...
app.get('/emprunts/membre/:id', async (req, res) => {
    try {
    const r = await
    axios.get(`${EMPRUNT_SERVICE}/emprunts/membre/${req.params.id}`)
    res.status(200).json(r.data)
    } catch (err) { res.status(500).json({ message: err.message }) }
    })
...
app.patch('/emprunts/:id/retour', async (req, res) => {
    try {
    const r = await
    axios.patch(`${EMPRUNT_SERVICE}/emprunts/${req.params.id}/retour`)
    res.status(200).json(r.data)
    } catch (err) {
    if (err.response) return
    res.status(err.response.status).json(err.response.data)
